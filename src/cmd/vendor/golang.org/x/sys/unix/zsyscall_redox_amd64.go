...
func PosixGetdents(fd int, buf []byte, flag int) (n int, err error) {
	...
	r0, _, e1 := sysvicall6(uintptr(unsafe.Pointer(&libc_posix_getdents)), 4, uintptr(fd), uintptr(unsafe.Pointer(_p0)), uintptr(len(buf)), uintptr(flag), 0, 0)
	runtime.KeepAlive(buf)
	n = int(r0)
	if e1 != 0 {
		err = errnoErr(e1)
